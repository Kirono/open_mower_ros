...
void statusCallback(const mower_msgs::Status::ConstPtr& msg) {
    if (msg->ultrasonic_ranges.size() >= 3) {
        publishScan(left_pub, msg->ultrasonic_ranges[1], "ultrasonic_left_frame",-0.35,0.55,2.5,false);
        publishScan(center_pub, msg->ultrasonic_ranges[2], "ultrasonic_center_frame",-0.5,0.5,2.5,false);
        publishScan(right_pub, msg->ultrasonic_ranges[3], "ultrasonic_right_frame",-0.55,0.35,2.5,true);
