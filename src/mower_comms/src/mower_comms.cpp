...
void publishScan(ros::Publisher& pub, float range, const std::string& frame_id, float anglemin, float anglemax, bool isRigthSensor) {
...
void statusCallback(const mower_msgs::Status::ConstPtr& msg) {
    if (msg->ultrasonic_ranges.size() >= 3) {
        publishScan(left_pub, msg->ultrasonic_ranges[1], "ultrasonic_left_frame",-0.25,0.25,false);
        publishScan(center_pub, msg->ultrasonic_ranges[2], "ultrasonic_center_frame",-0.5,0.5,false);
        publishScan(right_pub, msg->ultrasonic_ranges[3], "ultrasonic_right_frame",-0.25,0.25,true);
